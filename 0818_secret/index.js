...
const createPbkdf = (password) => {
    // pbkdf2함수(비밀번호, 솔트, 반복횟수, 키 길이, 알고리즘)으로 생성 되고
    // 반환되는 값은 Buffer값
...
const saltNumber = 10; // 암호화에 사용할 salt의 수준을 설정합니다. 정수사용
// 암호화
const bcryptPassword = (password) => {
    return bycrpt.hashSync(password, saltNumber);
};
// 비교
const comparePassword = (password, dbPassword) => {
    return bycrpt.compareSync(password, dbPassword);
};